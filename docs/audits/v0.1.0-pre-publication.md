...
**A historical record, superseded by events.** This was written on 2026-08-05, before the
repository was published, and it describes the state of the work at that moment. It is kept
unedited in substance because an audit that is rewritten to match later outcomes stops being
evidence of anything.

Everything it records as pending has since happened: the repository is public, `main` was
merged, and v0.1.0 onward are tagged and released. Later versions have their own audits in this
directory, and the current state is described by `README.md` and `CHANGELOG.md` rather than
here.
...
Every component here was written from a behavioral specification rather than adapted from
existing code. No reuse authorization was requested, because none was needed. The decision
procedure that would have applied had reuse been contemplated is in
`docs/CLEAN_ROOM_PLAN.md` §9.
...
```
authors        : one author, under two Git identities (a handle and a full name)
...
| 2 | Confirm the clean-room method in `docs/CLEAN_ROOM_PLAN.md` was followed in full | Author |
...
Everything in this list is verification rather than engineering work.
...
| Push the feature branch | **Held** — nothing had been pushed to the remote at the time |
| Merge into `main` | **Held** — `main` was untouched at the time |
| Public visibility | **Held** — the repository was private at the time |
| `v0.1.0` release | **Held** — no tag existed at the time |

All five were deliberate holds pending review of the completed work. All have since been
carried out.
